...
def main(args: argparse.Namespace):
    ...
    for epoch in range(args.epochs):
        ...
        if train_batch_loss == np.min(train_loss):
            torch.save(model, 'model/test.pt')
    # Draw loss & acc
    draw_regression_loss(train_loss, val_loss)
    save_result(train_loader, val_loader, model, json_file='test_data.json', device='cuda')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-vp', '--y_path', default='data/onehot_4classes.npy',
                        help='the path of y data.')

    # train parameters
    parser.add_argument('-bs', '--batch_size', type=int, default=16, help='the size of batch.')
    parser.add_argument('-ep', '--epochs', type=int, default=500, help='the num of epochs.')
    parser.add_argument('-ts', '--test_size', type=float, default=0.1, help='the percent of val_data.')
    ...
    if optuna_model != True:
        ...
        def objective(trial):
            ...
            for epoch in range(epochs):
                train_batch_loss, train_batch_acc = classification_train(epoch, train_loader, model, optimizer, criterion, args)
                train_loss = np.append(train_loss, train_batch_loss)
                # 1.2 classification_validate
                val_batch_loss, val_batch_acc = classification_validate(epoch, val_loader, model, criterion, args)
            ...
            np.save(f'trials_log/trials_{trials}_train_loss.npy', train_loss)
            np.save(f'trials_log/trials_{trials}_val_loss.npy', val_loss)

            save_result(train_loader, val_loader, model, json_file=f'trials_log/model_result_{trials}.json', device='cuda')
            goal = np.load('goal.npy')
            if train_acc[-1]+val_acc[-1] > goal:
                np.save('goal.npy', train_acc[-1]+val_acc[-1])
                torch.save(model, 'model/best_acc.pt')
